...
3. 打開 Dashboard 網址,把決定好的建議一筆一筆填入表單(代號/買賣/**股數**,單位是零股,1~999/**價格**,填方舟建議的淨值價,留空用參考價)新增
4. Dashboard 會列出「待確認清單」,同時顯示你目前的實際持股(股)方便比對
5. 確認沒問題,按「確認送出全部訂單」— 這時才會真的呼叫 Shioaji 用「盤中零股」下單(9:00~13:30,分批撮合)
6. 當天沒成交想改價,到「今日委託」區塊取消該筆,再用新價格重新新增一次
7. 下方「歷史紀錄」會自動累積每次的下單結果(含實際掛單價格、失敗原因)

## 目前狀態

- ✅ 程式邏輯完成(登入、比對持股、下單、歷史紀錄、取消委託),下單單位為零股(IntradayOdd)
- ✅ 已部署到 Zeabur(tzujen-ark-trade.zeabur.app)
- ✅ 永豐帳戶已完成簽署 + API 測試審核,**正式環境運作中**(SJ_SIMULATION=false)
- ✅ 下單價格會自動對齊跳價單位(ETF 用兩級距規則,一般股票用交易所六級距規則),避免因為價格不合法被拒單
- ℹ️ 永豐官方 API 測試(登入測試/下單測試/複委託合約排查)那組按鈕已經完成階段性任務,程式碼已移除
